...
- 自动审计不在知识库内生成 `*审计.md`；结果以终端 `PASS/FAIL` 为准，可重建的 JSON 明细由 `.gitignore` 排除。脚注 ID 内部前缀 `ovpNN` 是机器标识，不属于用户可见序号。
...
- 审计：`tools/audit_knowledge_graph.py`（仅输出终端 `PASS/FAIL`，不生成 Markdown 报告）
...
- 概念候选必须同时满足：位于 `Knowledge/Concepts/` 且 frontmatter `type: concept`。索引、教材与模板不得进入自动双链候选；审计 JSON 也不参与候选扫描。
...
- `git status` / `git diff --stat`；确认 `Raw Materials/`、`AGENTS.md`、`__pycache__/` 与 `.obsidian/` 本机状态未进入提交。根目录 `.obsidian/graph.json` 仅可在共享关系图颜色分组确有变化时提交，主题、外观、工作区与插件状态不得进入提交。
