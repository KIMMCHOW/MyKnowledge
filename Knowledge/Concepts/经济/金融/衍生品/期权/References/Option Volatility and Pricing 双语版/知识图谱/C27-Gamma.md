...
- **决定Delta漂移速度 →** [[C26-Delta|C26 Delta]]：Gamma 把标的变动传导为 Delta 变化。
- **提高再平衡需求 →** [[C33-动态Delta对冲|C33 动态Delta对冲]]：高 Gamma 使对冲更快失衡。
- **通常伴随反向暴露 →** [[C28-Theta|C28 Theta]]：同一模型条件下正 Gamma 通常伴随负 Theta。
- **汇总为组合暴露 →** [[C32-组合Greeks与头寸分析|C32 组合Greeks与头寸分析]]：各腿 Gamma 形成组合凸性。

### 受上游约束

- [[C22-Black-Scholes模型|C22 Black-Scholes模型]] **—导出局部敏感度→ 本概念**：模型对标的价格求二阶敏感度。
- [[C23-二叉树与风险中性估值|C23 二叉树与风险中性估值]] **—以节点差分估计→ 本概念**：相邻 Delta 变化可估计 Gamma。
- [[C35-跨式与宽跨式|C35 跨式与宽跨式]] **—多头通常形成正凸性→ 本概念**：多头跨式和宽跨式受益于大幅运动。
- [[C36-蝶式与鹰式|C36 蝶式与鹰式]] **—表达局部曲率→ 本概念**：多执行价结构把 Gamma 集中在特定区间。
- [[C38-比率价差与圣诞树|C38 比率价差与圣诞树]] **—可能形成非对称尾部凸性→ 本概念**：不等数量使一侧远端风险突出。
