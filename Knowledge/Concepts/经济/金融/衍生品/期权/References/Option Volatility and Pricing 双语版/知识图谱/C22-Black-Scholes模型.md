...
# C22 Black-Scholes模型 / Black-Scholes Model
...
- **计算 →** [[C21-期望值与理论价值|C21 期望值与理论价值]]：Black–Scholes 把输入映射为理论价值。
- **提供反解机制 →** [[C17-隐含波动率|C17 隐含波动率]]：隐波是模型依赖的报价坐标。
- **导出局部敏感度 →** [[C26-Delta|C26 Delta]]：模型对标的价格求一阶敏感度。
- **导出局部敏感度 →** [[C27-Gamma|C27 Gamma]]：模型对标的价格求二阶敏感度。
- **导出局部敏感度 →** [[C28-Theta|C28 Theta]]：模型衡量时间流逝的局部影响。
- **导出局部敏感度 →** [[C29-Vega|C29 Vega]]：模型衡量波动率输入变化的局部影响。
- **导出局部敏感度 →** [[C30-Rho|C30 Rho]]：模型衡量利率输入变化的局部影响。

### 受上游约束

- [[C16-波动率预测|C16 波动率预测]] **—提供波动率假设→ 本概念**：预测波动率可作为正向估值输入。
